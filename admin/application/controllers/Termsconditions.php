...
class Termsconditions  extends CI_Controller {
    ...
    public function index() {
            $data['currentPage'] = "termsconditions";
            $this->load->view('termsconditions/radio-list',$data);
    }
       ...
       public function add() {
            ...
            $this->load->view('termsconditions/addRadioView',$data);
    }
    ...
    public function edit($mediaID=NULL){
        ...
        $categoriesList=$this->Movie_model->getCategoriesList("3");
        $data['categoriesList']=$categoriesList;
        $filter['termid'] = $mediaID;
        if($getData = $this->General_model->getData("termsconditions",$filter)){
            $data['media_detail']= $getData ;
        }else{
          redirect('Termsconditions/');
        }
     ...
     public function add_radio(){
      ...
      if($radio_id == ""){
        $radioID = $this->General_model->insert('termsconditions', $insertdata);
      }else{

        $filterD['termid'] =  $radio_id;
        $this->General_model->update('termsconditions', $filterD,$insertdata);
        $radioID = $radio_id;
      }
        ...
        else
        {
         ...
         $data = $this->db->query("SELECT * FROM termsconditions where description LIKE '%".$search."%' ORDER BY $tbl.$columnName $order LIMIT ".$start." , ".$length." ");



        $result = $data->result();
        foreach ($result as $key => $value) {
          $result[$key]->DT_RowId = "DT_RowId_".$result[$key]->termid;
        ...
        $data12 = $this->db->query("SELECT * FROM termsconditions");
     ...
     public function radioActivation(){
         ...
         if($this->input->is_ajax_request()){
           ...
           $id = $this->webservice_general_model->delete('termsconditions',$filter,$setData);
            if($id){
              echo 1; die;
          }else{
              echo 0; die;
  ...
  public function languageActivation(){
         ...
         if($this->input->is_ajax_request()){
           ...
           $id = $this->webservice_general_model->update('termsconditions',$filter,$setData);
            if($id){
              echo 1; die;
          }else{
              echo 0; die;
