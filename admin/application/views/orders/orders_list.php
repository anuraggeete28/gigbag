    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container">
          <div class="row">
              <div class="col-xs-12 col-sm-12 col-md-12">
                <form action="" method="post" id="orderStatusUpdateForm">
                  <input type="hidden" name="id" value="" id="orderId" />
                  <input type="hidden" name="rowId" value="" id="rowId" />
                  <div class="form-group">
                    <label>Order Status</label>
                    <select name="order_status" id="orderStatusDropdown" class="form-control" aria-label="Default select example">
...
$(document).ready(function() {
     var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>orders/orderListwebAPI",// The service URL
 ...
 $(document).on('click','.delete',function(){
  ...
  $("#orderStatusUpdateForm").submit(function(e) {
      e.preventDefault();
      var form = $(this);
      var actionUrl = form.attr('action');
      $.ajax({
      type:'post',
      url : '<?php echo base_url('Orders/orderStatusUpdate'); ?>',
      data: form.serialize(),
      success : function(response){
        response = JSON.parse(response);
        if(response.status) {
          $('#'+response.data.rowId).children('td').eq(6).text(response.data.order_status);
        }
