    ...
    border: 1px solid;
            ...
            <div class="container-fluid">
                <div class="row">
                 ...
                 <a href="<?php echo base_url() ?>Privacypolicy/add" class="btn btn-primary">Add Privacy Policy</a>
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>Privacypolicy/radioListwebAPI", // The service URL
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.qr_image= "<img src="+object.qr_image+ " alt='Image Not Available' height='55' width='100'>";
                         if(object.isActive == 0){
                              object.action = "<button  onclick='deactiveRadio("+object.policyid+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }else{
                            object.action = "<button  onclick='activeRadio("+object.policyid+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }
                        object.edit = "<a href='<?php echo base_url() ?>Privacypolicy/edit/"+object.policyid+"'><i class='fa fa-edit fa-lg'></i></a>";
                        
                        if(object.isStatus == 0){
                              object.action2 = "<button  onclick='deactiveLanguage("+object.policyid+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Un-Approve</button>";
                        }else{
                            object.action2 = "<button  onclick='activeLanguage("+object.policyid+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Approve</button>";
                        }
                      }
                      return json.data;
                  },
     ...
     function activeRadio(ID){
              if(ID != ''){
               var res = confirm("Do you want to delete this Privacy policy ?");
               if (res == true) {

                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('Privacypolicy/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Un-Approve this Privacy policy ?");
           if (res == true) {

              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Privacypolicy/languageActivation/'); ?>',
      ...
      function deactiveLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Approve this Privacy policy ?");
           if (res == true) {
