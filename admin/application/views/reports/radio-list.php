...
<a href="<?php echo base_url() ?>uploads/GMIreport.csv" class="btn btn-primary rrrr">Download Sample</a>
                 <a href="<?php echo base_url() ?>Reports/add" class="btn btn-primary">Add Report Manually</a>
                        <div class="card">
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>Reports/radioListwebAPI", // The service URL
                  ...
                  dataSrc: function (json)
                  {
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.edit = "<a href='<?php echo base_url() ?>Reports/edit/"+object.data_id+"'><i class='fa fa-edit fa-lg'></i></a>";
                        
                        if(object.isStatus == 0){
                              object.action2 = "<button  onclick='deactiveLanguage("+object.data_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Un-Approve</button>";
                        }else{
     ...
     function activeRadio(ID){
              if(ID != ''){
               var res = confirm("Do you want to delete this report ?");
               if (res == true) {

                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('Reports/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Un-Approve this report ?");
           if (res == true) {

              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Reports/languageActivation/'); ?>',
      ...
      function deactiveLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Approve this report ?");
           if (res == true) {
