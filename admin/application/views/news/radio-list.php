          ...
          .paginate_button{
            ...
            <div class="container-fluid">
                <div class="row">
                 ...
                 <a href="<?php echo base_url() ?>News/add" class="btn btn-primary">Add News</a>
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>News/radioListwebAPI", // The service URL
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.qr_image= "<img src="+object.qr_image+ " alt='Image Not Available' height='55' width='100'>";
                         if(object.isActive == 0){
                              object.action = "<button  onclick='deactiveRadio("+object.news_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }else{
                            object.action = "<button  onclick='activeRadio("+object.news_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }
                        object.edit = "<a href='<?php echo base_url() ?>News/edit/"+object.news_id+"'><i class='fa fa-edit fa-lg'></i></a>";
                        
                        if(object.isStatus == 0){
                              object.action2 = "<button  onclick='deactiveLanguage("+object.news_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Un-Approve</button>";
                        }else{
                            object.action2 = "<button  onclick='activeLanguage("+object.news_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Approve</button>";
                        }
                      }
                      return json.data;
                  },
     ...
     function activeRadio(ID){
              if(ID != ''){
               ...
               if (res == true) {
                  ...
                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('News/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           ...
           if (res == true) {
              ...
              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('News/languageActivation/'); ?>',
    ...
    function getLanguage2(ID){
        $("#lang_id").val(ID);
          if(ID != ''){
              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('News/ordersdetail/'); ?>',
                        data : {'ID': ID},
                        success : function(data){
