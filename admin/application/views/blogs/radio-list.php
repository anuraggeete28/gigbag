          ...
          .paginate_button{
            ...
            <div class="container-fluid">
                <div class="row">
                 ...
                 <a href="<?php echo base_url() ?>Blogs/add" class="btn btn-primary">Add Blog</a>
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>Blogs/radioListwebAPI", // The service URL
                  ...
                  dataSrc: function (json)
                  {
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.blogimage= "<img src="+object.blogimage+ " alt='Image Not Available' height='55' width='100'>";
                        ...
                        object.authorimg= "<img src="+object.authorimg+ " alt='Image Not Available' height='55' width='100'>";
                         if(object.isActive == 0){
                              object.action = "<button  onclick='deactiveRadio("+object.blog_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }else{
                            object.action = "<button  onclick='activeRadio("+object.blog_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }
                        object.edit = "<a href='<?php echo base_url() ?>Blogs/edit/"+object.blog_id+"'><i class='fa fa-edit fa-lg'></i></a>";
                        
                        if(object.isStatus == 0){
                              object.action2 = "<button  onclick='deactiveLanguage("+object.blog_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Un-Approve</button>";
                        }else{
                            object.action2 = "<button  onclick='activeLanguage("+object.blog_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Approve</button>";
                        }
                      }
                      return json.data;
                  },
     ...
     function activeRadio(ID){
              if(ID != ''){
               var res = confirm("Do you want to delete this Blog ?");
               if (res == true) {

                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('Blogs/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Un-Approve this Blog ?");
           if (res == true) {

              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Blogs/languageActivation/'); ?>',
      ...
      function deactiveLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Approve this Blog ?");
           if (res == true) {
    ...
    function getLanguage2(ID){
        $("#lang_id").val(ID);
          if(ID != ''){
              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Blogs/ordersdetail/'); ?>',
                        data : {'ID': ID},
                        success : function(data){
