            ...
            <div class="row page-titles mx-0">
                <div class="col p-md-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="javascript:void(0)">Custmize Sample Request</a></li>
                    </ol>
                </div>
            </div>
            <!-- row -->
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>Custmizesample/radioListwebAPI", // The service URL
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.course_img= "<img src="+object.course_img+ " alt='Image Not Available' height='55' width='100'>";
                         if(object.isActive == 0){
                              object.action = "<button  onclick='deactiveRadio("+object.cust_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }else{
                            object.action = "<button  onclick='activeRadio("+object.cust_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }
                        object.edit = "<a href='<?php echo base_url() ?>Custmizesample/edit/"+object.cust_id+"'><i class='fa fa-edit fa-lg'></i></a>";
                        ...
                        if(object.isStatus == 0){
                            ...
                            object.action2 = "<button  onclick='activeLanguage("+object.cust_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Approve</button>";
                        }
                      }
                      return json.data;
                  },
     ...
     function activeRadio(ID){
              if(ID != ''){
               var res = confirm("Do you want to delete this Custmizesample ?");
               if (res == true) {

                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('Custmizesample/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Un-Approve this Custmizesample ?");
           if (res == true) {

              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Custmizesample/languageActivation/'); ?>',
      ...
      function deactiveLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Approve this Custmizesample ?");
           if (res == true) {
