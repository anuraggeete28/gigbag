...
<div class="container-fluid">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <a href="<?php echo base_url('Categories/add/'.$category_id) ?>" class="btn btn-primary">Add Subcategory</a>
          <?php 
  ...
  <style>
                ...
                <div class="row csvdownload">
                    <button class="btn btn-primary"><a style = "color:white;margin-top:-10px;"href='<?= base_url() ?>Exportsubcat/export'>Download Sub Category Data</a></button>
                  ...
                  <table id="example" class="table table table-bordered" style="width:100%">
                    <thead>
                      <tr>
                        ...
                        <th>Categories</th>
                        <th>Subcategories</th>
                        <th style="background: #fff;">Picture</th>
                        <th>edit</th>  
                        <th>Action</th>
                      </tr>
                    </thead>
...
$(document).ready(function() {
    ...
    var tbl_productList = $('#example').DataTable(
      ...
      "processing": false,
              ...
              "order": [],
               ajax: {
                  url: "<?php echo base_url() ?>Categories/radioListwebAPI", // The service URL
                      ...
                      for (var i = 0; i < json.data.length; i++) 
                      {
                        ...
                        object.qr_image= "<img src="+object.qr_image+ " alt='Image Not Available' height='55' width='100'>";
                         if(object.isActive == 0){
                              object.action = "<button  onclick='deactiveRadio("+object.subcat_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }else{
                            object.action = "<button  onclick='activeRadio("+object.subcat_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Delete</button>";
                        }
                        object.edit = "<a href='<?php echo base_url() ?>Categories/edit/"+object.subcat_id+"'><i class='fa fa-edit fa-lg'></i></a>";
                        
                        if(object.isStatus == 0){
                              object.action2 = "<button  onclick='deactiveLanguage("+object.subcat_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Un-Approve</button>";
                        }else{
                            object.action2 = "<button  onclick='activeLanguage("+object.subcat_id+");' href='javascript:void(0);' type='button' class='btn btn-primary'>Approve</button>";
                        }
                      }
                      return json.data;
                  },
     ...
     function activeRadio(ID){
              if(ID != ''){
               ...
               if (res == true) {
                  ...
                  $.ajax({
                            type:'post',
                            url : '<?php echo base_url('Categories/radioActivation/'); ?>',
        ...
        function activeLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Un-Approve this Category ?");
           if (res == true) {

              $.ajax({
                        type:'post',
                        url : '<?php echo base_url('Categories/languageActivation/'); ?>',
      ...
      function deactiveLanguage(ID){
          if(ID != ''){
           var res = confirm("Do you want to Approve this Category ?");
           if (res == true) {
